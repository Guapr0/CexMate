...
PROMPT_EVERY_RUN = """  From the project root, read and analyze `output/raw_facebook_list.json` carefully and accurately. Create/overwrite `output/organized_facebook_list.json` containing a JSON array with one object per listing.
  ...
  `brand`, `model`, `variant`, `color`, `storage_gb`, `ram_gb`, `dual_sim`, `battery_health_percent`, `accessories_included`, `condition`, `grade`, `carrier`, `price`, 'market_price', `location`, `recency`, `image`,
  `fb_link`, 'cex_link', `description`.

  Rules:
  - If a value is missing or unclear, set it to `null`. Do not omit any keys.
  - `market_price` and `cex_link` must always be set to `null`.
  ...
  carrier, battery health). If not explicitly stated, set them to `null`, except for the special storage rule below.
